...
def callback(in_data, frames, total_time, status):
    """
  ...
  """
    ...
    if status:
        return
    if any(in_data):
        callback.window_samples = np.concatenate((callback.window_samples, in_data[:, 0]))  # append new samples
        callback.window_samples = callback.window_samples[len(in_data[:, 0]):]  # remove old samples
        ...
        for j in range(len(Constants.OCTAVE_BANDS) - 1):
            ind_start = int(Constants.OCTAVE_BANDS[j] / DELTA_FREQ)
            ind_end = int(Constants.OCTAVE_BANDS[j + 1] / DELTA_FREQ)
        ...
        if callback.noteBuffer.count(callback.noteBuffer[0]) == len(callback.noteBuffer):
            ...
            Constants.PITCH_LIST.append(max_freq)
            Constants.CURRENT_PITCH = max_freq
            Constants.NUM_OF_ITERATION = Constants.NUM_OF_ITERATION + 1


def tuner():
    try:
        with sd.InputStream(channels=1, callback=callback, blocksize=WINDOW_STEP, samplerate=SAMPLE_FREQ):
            while Constants.NUM_OF_ITERATION < 7:
                time.sleep(0.5)
